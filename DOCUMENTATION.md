...
* [5. Setting Up Client Network](#5-setting-up-client-network)
  ...
  * [5.8. Public IP Address Detection](#58-public-ip-address-detection)
...
## 5.8. Public IP Address Detection

Whenever Hazelcast cluster members are able to resolve their own public external IP addresses, they pass this information to the client. As a result, the client can use public addresses for communication, if it cannot access members via private IPs. This may be important in a Cloud deployment, for example, when you set up a Hazelcast cluster on Kubernetes and would like to use it from outside Kubernetes (or from a different Kubernetes cluster).
...
You can disable this feature and specify the client behavior by using the following `properties` config option:
...
> **NOTE: Note that, this detection mechanism is disabled when the client is configured to use [TLS/SSL encryption](#81-tlsssl). In such setup you should explicitly set the `hazelcast.discovery.public.ip.enabled` property.**
