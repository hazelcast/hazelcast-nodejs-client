...
var AUTH_STATUS_INVALID_CREDENTIALS = 1;
var AUTH_STATUS_SERIALIZATION_MISMATCH = 2;
...
var HazelcastClient = function (options) {
    ...
    this.connection = net.connect({"host": host, "port": port}, function () {
        ...
        self.authenticate(username, password).then(function (response) {
            ...
            if (response.status == AUTH_STATUS_INVALID_CREDENTIALS) {
                ready.reject("Invalid credentials");
                return;
            }

            if (response.status == AUTH_STATUS_SERIALIZATION_MISMATCH) {
