...
var Auth = require("./codec/Client_authentication_codec");
...
HazelcastClient.prototype.authenticate = function (username, password) {
    var auth = new Auth();
    var client_message = auth.encode_request(username,password,null,null,true,'node',1);
    var correlationId = this.correlationCounter++;
    client_message.set_correlation_id(correlationId);
    client_message.set_partition_id(-1);
    client_message.set_flags(0xc0);

    this.connection.write(client_message.buffer);
