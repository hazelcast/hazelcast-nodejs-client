...
export class MapAddNearCacheInvalidationListenerCodec {
    ...
    private static EVENT_I_MAP_INVALIDATION_SEQUENCE_FIELD_OFFSET = MapAddNearCacheInvalidationListenerCodec.EVENT_I_MAP_INVALIDATION_PARTITION_UUID_FIELD_OFFSET + FixedSizeTypes.UUID_SIZE_IN_BYTES;
    private static EVENT_I_MAP_INVALIDATION_INITIAL_FRAME_SIZE = MapAddNearCacheInvalidationListenerCodec.EVENT_I_MAP_INVALIDATION_SEQUENCE_FIELD_OFFSET + FixedSizeTypes.LONG_SIZE_IN_BYTES;
    // hex: 0x014502
    private static EVENT_I_MAP_INVALIDATION_MESSAGE_TYPE = 83202;
    ...
    // hex: 0x014503
    private static EVENT_I_MAP_BATCH_INVALIDATION_MESSAGE_TYPE = 83203;
    ...
    static encodeRequest(name: string, listenerFlags: number, localOnly: boolean): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapAddNearCacheInvalidationListenerCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: string ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapAddNearCacheInvalidationListenerCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeIMapInvalidationEvent( key: Data,  sourceUuid: string,  partitionUuid: UUID,  sequence: Long): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapAddNearCacheInvalidationListenerCodec.EVENT_I_MAP_INVALIDATION_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeIMapBatchInvalidationEvent( keys: Array<Data>,  sourceUuids: Array<string>,  partitionUuids: Array<UUID>,  sequences: Array<Long>): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapAddNearCacheInvalidationListenerCodec.EVENT_I_MAP_BATCH_INVALIDATION_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        initialFrame.flags |= ClientMessage.IS_EVENT_FLAG;
    ...
    static handle(clientMessage: ClientMessage,  handleIMapInvalidation: any, handleIMapBatchInvalidation: any, toObjectFunction: (data: Data) => any = null): void {
        ...
        const frame: Frame = clientMessage.get();
        const initialFrame: Frame = frame.next;
        const partitionUuid: UUID  = FixedSizeTypes.decodeUUID(initialFrame.content, MapAddNearCacheInvalidationListenerCodec.EVENT_I_MAP_INVALIDATION_PARTITION_UUID_FIELD_OFFSET);
        const sequence: Long  = FixedSizeTypes.decodeLong(initialFrame.content, MapAddNearCacheInvalidationListenerCodec.EVENT_I_MAP_INVALIDATION_SEQUENCE_FIELD_OFFSET);
