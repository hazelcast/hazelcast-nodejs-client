...
export class DynamicConfigAddRingbufferConfigCodec {
    ...
    static encodeRequest(name: string, capacity: number, backupCount: number, asyncBackupCount: number, timeToLiveSeconds: number, inMemoryFormat: string, ringbufferStoreConfig: RingbufferStoreConfigHolder, quorumName: string, mergePolicy: string, mergeBatchSize: number) {
        var clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName("DynamicConfig.AddRingbufferConfig");
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(DynamicConfigAddRingbufferConfigCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse() {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(DynamicConfigAddRingbufferConfigCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
