...
export class CacheAddEntryListenerCodec {
    ...
    static EVENT_CACHE_TYPE_FIELD_OFFSET = ClientMessage.PARTITION_ID_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    static EVENT_CACHE_COMPLETION_ID_FIELD_OFFSET = CacheAddEntryListenerCodec.EVENT_CACHE_TYPE_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    static EVENT_CACHE_INITIAL_FRAME_SIZE = CacheAddEntryListenerCodec.EVENT_CACHE_COMPLETION_ID_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    //hex: 0x150102
    static EVENT_CACHE_MESSAGE_TYPE = 1376514;
    ...
    static encodeRequest(name: string, localOnly: boolean) {
        ...
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(CacheAddEntryListenerCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse(response: string ) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(CacheAddEntryListenerCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeCacheEvent( type : number,  keys : Array<CacheEventData>,  completionId : number) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(CacheAddEntryListenerCodec.EVENT_CACHE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
