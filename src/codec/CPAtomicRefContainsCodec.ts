...
export class CPAtomicRefContainsCodec {
    ...
    private CPAtomicRefContainsCodec() {
    }


    static encodeRequest(groupId: RaftGroupId, name: string, value: Data) {
        var clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(true);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName("CPAtomicRef.Contains");
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(CPAtomicRefContainsCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse(response: boolean ) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(CPAtomicRefContainsCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
