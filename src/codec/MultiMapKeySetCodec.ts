...
export class MultiMapKeySetCodec {
    // hex: 0x020400
    public static REQUEST_MESSAGE_TYPE = 132096;
    // hex: 0x020401
    public static RESPONSE_MESSAGE_TYPE = 132097;
    ...
    static encodeRequest(name: string): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MultiMapKeySetCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MultiMapKeySetCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: Array<Data> ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MultiMapKeySetCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MultiMapKeySetCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
