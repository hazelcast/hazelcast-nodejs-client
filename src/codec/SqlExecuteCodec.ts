...
const RESPONSE_PARTITION_ARGUMENT_INDEX_OFFSET = RESPONSE_IS_INFINITE_ROWS_OFFSET + BitsUtil.BOOLEAN_SIZE_IN_BYTES;

/** @internal */
export interface SqlExecuteResponseParams {
    rowMetadata: SqlColumnMetadataImpl[] | null;
    ...
    isPartitionArgumentIndexExists: boolean;
}

/** @internal */
export class SqlExecuteCodec {
    static encodeRequest(sql: string, parameters: Data[], timeoutMillis: Long, cursorBufferSize: number, schema: string | null , expectedResultType: number, queryId: SqlQueryId, skipUpdateStatistics: boolean): ClientMessage {
    ...
    static decodeResponse(clientMessage: ClientMessage): SqlExecuteResponseParams {
        ...
        if (initialFrame.content.length >= RESPONSE_PARTITION_ARGUMENT_INDEX_OFFSET + BitsUtil.INT_SIZE_IN_BYTES) {
            response.partitionArgumentIndex = FixSizedTypesCodec.decodeInt(initialFrame.content, RESPONSE_PARTITION_ARGUMENT_INDEX_OFFSET);
