...
export class ClientDestroyProxyCodec {
    // hex: 0x000600
    public static REQUEST_MESSAGE_TYPE = 1536;
    // hex: 0x000601
    public static RESPONSE_MESSAGE_TYPE = 1537;
    ...
    static encodeRequest(name: string, serviceName: string): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientDestroyProxyCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ClientDestroyProxyCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientDestroyProxyCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ClientDestroyProxyCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
