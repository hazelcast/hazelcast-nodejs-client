...
export class ScheduledExecutorCancelFromPartitionCodec {
    ...
    static encodeRequest(schedulerName: string, taskName: string, mayInterruptIfRunning: boolean): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(true);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName('ScheduledExecutor.CancelFromPartition');
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ScheduledExecutorCancelFromPartitionCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: boolean ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ScheduledExecutorCancelFromPartitionCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
