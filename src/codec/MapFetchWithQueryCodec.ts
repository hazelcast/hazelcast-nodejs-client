...
export class MapFetchWithQueryCodec {
    ...
    static encodeRequest(name: string, tableIndex: number, batch: number, projection: Data, predicate: Data): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(true);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName('Map.FetchWithQuery');
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapFetchWithQueryCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(results: Array<Data> , nextTableIndexToReadFrom: number ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapFetchWithQueryCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
