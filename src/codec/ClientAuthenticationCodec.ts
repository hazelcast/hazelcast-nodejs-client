...
export class ClientAuthenticationCodec {
    ...
    static encodeRequest(username: string, password: string, uuid: string, ownerUuid: string, isOwnerConnection: boolean, clientType: string, serializationVersion: number, clientHazelcastVersion: string, clientName: string, labels: Array<string>, partitionCount: number, clusterId: string): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(true);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName('Client.Authentication');
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientAuthenticationCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(status: number , address: Address , uuid: string , ownerUuid: string , serializationVersion: number , serverHazelcastVersion: string , clientUnregisteredMembers: Array<Member> , partitionCount: number , clusterId: string ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientAuthenticationCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
