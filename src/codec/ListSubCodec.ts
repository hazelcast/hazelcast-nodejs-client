...
export class ListSubCodec {
    // hex: 0x051500
    public static REQUEST_MESSAGE_TYPE = 333056;
    // hex: 0x051501
    public static RESPONSE_MESSAGE_TYPE = 333057;
    private static REQUEST_FROM_FIELD_OFFSET = ClientMessage.PARTITION_ID_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    private static REQUEST_TO_FIELD_OFFSET = ListSubCodec.REQUEST_FROM_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    private static REQUEST_INITIAL_FRAME_SIZE = ListSubCodec.REQUEST_TO_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    ...
    static encodeRequest(name: string, from: number, to: number): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ListSubCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ListSubCodec.REQUEST_MESSAGE_TYPE);
        FixedSizeTypes.encodeInt(initialFrame.content, ListSubCodec.REQUEST_FROM_FIELD_OFFSET, from);
        FixedSizeTypes.encodeInt(initialFrame.content, ListSubCodec.REQUEST_TO_FIELD_OFFSET, to);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
        ...
        request.from =  FixedSizeTypes.decodeInt(initialFrame.content, ListSubCodec.REQUEST_FROM_FIELD_OFFSET);
        request.to =  FixedSizeTypes.decodeInt(initialFrame.content, ListSubCodec.REQUEST_TO_FIELD_OFFSET);
     ...
     static encodeResponse(response: Array<Data> ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ListSubCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ListSubCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
