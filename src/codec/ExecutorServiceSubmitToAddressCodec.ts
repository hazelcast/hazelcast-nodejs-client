...
export class ExecutorServiceSubmitToAddressCodec {
    //hex: 0x090600
    public static REQUEST_MESSAGE_TYPE = 591360;
    //hex: 0x090601
    public static RESPONSE_MESSAGE_TYPE = 591361;
    ...
    static encodeRequest(name: string, uuid: string, callable: Data, address: Address) {
        ...
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(ExecutorServiceSubmitToAddressCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ExecutorServiceSubmitToAddressCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse(response: Data ) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(ExecutorServiceSubmitToAddressCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ExecutorServiceSubmitToAddressCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
