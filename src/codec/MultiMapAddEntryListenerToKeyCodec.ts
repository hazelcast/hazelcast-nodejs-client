...
export class MultiMapAddEntryListenerToKeyCodec {
    ...
    static EVENT_ENTRY_NUMBER_OF_AFFECTED_ENTRIES_FIELD_OFFSET = MultiMapAddEntryListenerToKeyCodec.EVENT_ENTRY_EVENT_TYPE_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    static EVENT_ENTRY_INITIAL_FRAME_SIZE = MultiMapAddEntryListenerToKeyCodec.EVENT_ENTRY_NUMBER_OF_AFFECTED_ENTRIES_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    //hex: 0x020D02
    static EVENT_ENTRY_MESSAGE_TYPE = 134402;

    private MultiMapAddEntryListenerToKeyCodec() {
    }


    static encodeRequest(name: string, key: Data, includeValue: boolean, localOnly: boolean) {
        var clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName("MultiMap.AddEntryListenerToKey");
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(MultiMapAddEntryListenerToKeyCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse(response: string ) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(MultiMapAddEntryListenerToKeyCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeEntryEvent( key : Data,  value : Data,  oldValue : Data,  mergingValue : Data,  eventType : number,  uuid : string,  numberOfAffectedEntries : number) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(MultiMapAddEntryListenerToKeyCodec.EVENT_ENTRY_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
...
static handle(clientMessage : ClientMessage, handleEventEntry: any, toObjectFunction: (data: Data) => any = null) {
            ...
            if (messageType == MultiMapAddEntryListenerToKeyCodec.EVENT_ENTRY_MESSAGE_TYPE) {
                var initialFrame : Frame = frame.next;
                var eventType : number  = FixedSizeTypes.decodeInt(initialFrame.content, MultiMapAddEntryListenerToKeyCodec.EVENT_ENTRY_EVENT_TYPE_FIELD_OFFSET);
                var numberOfAffectedEntries : number  = FixedSizeTypes.decodeInt(initialFrame.content, MultiMapAddEntryListenerToKeyCodec.EVENT_ENTRY_NUMBER_OF_AFFECTED_ENTRIES_FIELD_OFFSET);
