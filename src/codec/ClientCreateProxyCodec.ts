...
export class ClientCreateProxyCodec {
    // hex: 0x000500
    public static REQUEST_MESSAGE_TYPE = 1280;
    // hex: 0x000501
    public static RESPONSE_MESSAGE_TYPE = 1281;
    ...
    static encodeRequest(name: string, serviceName: string, target: Address): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientCreateProxyCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ClientCreateProxyCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientCreateProxyCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ClientCreateProxyCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
