...
export class DynamicConfigAddReliableTopicConfigCodec {
    ...
    static encodeRequest(name: string, listenerConfigs: Array<ListenerConfigHolder>, readBatchSize: number, statisticsEnabled: boolean, topicOverloadPolicy: string, executor: Data): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName('DynamicConfig.AddReliableTopicConfig');
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(DynamicConfigAddReliableTopicConfigCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(DynamicConfigAddReliableTopicConfigCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
