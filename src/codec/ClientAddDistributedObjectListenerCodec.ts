...
export class ClientAddDistributedObjectListenerCodec {
    ...
    private static EVENT_DISTRIBUTED_OBJECT_INITIAL_FRAME_SIZE = ClientMessage.PARTITION_ID_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    // hex: 0x000D02
    private static EVENT_DISTRIBUTED_OBJECT_MESSAGE_TYPE = 3330;
    ...
    static encodeRequest(localOnly: boolean): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientAddDistributedObjectListenerCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: string ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientAddDistributedObjectListenerCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeDistributedObjectEvent( name: string,  serviceName: string,  eventType: string): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientAddDistributedObjectListenerCodec.EVENT_DISTRIBUTED_OBJECT_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
