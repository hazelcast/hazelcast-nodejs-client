...
export class ListAddCodec {
    // hex: 0x050400
    public static REQUEST_MESSAGE_TYPE = 328704;
    // hex: 0x050401
    public static RESPONSE_MESSAGE_TYPE = 328705;
    ...
    private static RESPONSE_INITIAL_FRAME_SIZE = ListAddCodec.RESPONSE_RESPONSE_FIELD_OFFSET + FixedSizeTypes.BOOLEAN_SIZE_IN_BYTES;
    ...
    static encodeRequest(name: string, value: Data): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ListAddCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ListAddCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: boolean ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ListAddCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ListAddCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);

        FixedSizeTypes.encodeBoolean(initialFrame.content, ListAddCodec.RESPONSE_RESPONSE_FIELD_OFFSET, response);
        return clientMessage;
    }
     ...
     static decodeResponse(clientMessage: ClientMessage): ResponseParameters {
        ...
        response.response =  FixedSizeTypes.decodeBoolean(initialFrame.content, ListAddCodec.RESPONSE_RESPONSE_FIELD_OFFSET);
        return response;
    }
}
