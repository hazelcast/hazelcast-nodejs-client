...
export class CacheAddPartitionLostListenerCodec {
    ...
    static EVENT_CACHE_PARTITION_LOST_PARTITION_ID_FIELD_OFFSET = ClientMessage.PARTITION_ID_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    static EVENT_CACHE_PARTITION_LOST_INITIAL_FRAME_SIZE = CacheAddPartitionLostListenerCodec.EVENT_CACHE_PARTITION_LOST_PARTITION_ID_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    //hex: 0x151A02
    static EVENT_CACHE_PARTITION_LOST_MESSAGE_TYPE = 1382914;
    ...
    static encodeRequest(name: string, localOnly: boolean) {
        ...
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(CacheAddPartitionLostListenerCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse(response: string ) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(CacheAddPartitionLostListenerCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeCachePartitionLostEvent( partitionId : number,  uuid : string) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(CacheAddPartitionLostListenerCodec.EVENT_CACHE_PARTITION_LOST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
