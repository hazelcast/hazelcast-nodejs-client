...
export class ReplicatedMapAddEntryListenerCodec {
    ...
    static EVENT_ENTRY_NUMBER_OF_AFFECTED_ENTRIES_FIELD_OFFSET = ReplicatedMapAddEntryListenerCodec.EVENT_ENTRY_EVENT_TYPE_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    static EVENT_ENTRY_INITIAL_FRAME_SIZE = ReplicatedMapAddEntryListenerCodec.EVENT_ENTRY_NUMBER_OF_AFFECTED_ENTRIES_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    //hex: 0x0E0D02
    static EVENT_ENTRY_MESSAGE_TYPE = 920834;
    ...
    static encodeRequest(name: string, localOnly: boolean) {
        ...
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(ReplicatedMapAddEntryListenerCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse(response: string ) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(ReplicatedMapAddEntryListenerCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeEntryEvent( key : Data,  value : Data,  oldValue : Data,  mergingValue : Data,  eventType : number,  uuid : string,  numberOfAffectedEntries : number) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(ReplicatedMapAddEntryListenerCodec.EVENT_ENTRY_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
...
static handle(clientMessage : ClientMessage,  handleEntry: any, toObjectFunction: (data: Data) => any = null) {
            ...
            if (messageType == ReplicatedMapAddEntryListenerCodec.EVENT_ENTRY_MESSAGE_TYPE) {
                var initialFrame : Frame = frame.next;
                var eventType : number  = FixedSizeTypes.decodeInt(initialFrame.content, ReplicatedMapAddEntryListenerCodec.EVENT_ENTRY_EVENT_TYPE_FIELD_OFFSET);
                var numberOfAffectedEntries : number  = FixedSizeTypes.decodeInt(initialFrame.content, ReplicatedMapAddEntryListenerCodec.EVENT_ENTRY_NUMBER_OF_AFFECTED_ENTRIES_FIELD_OFFSET);
