...
export class MultiMapValuesCodec {
    // hex: 0x020500
    public static REQUEST_MESSAGE_TYPE = 132352;
    // hex: 0x020501
    public static RESPONSE_MESSAGE_TYPE = 132353;
    ...
    static encodeRequest(name: string): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MultiMapValuesCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MultiMapValuesCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: Array<Data> ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MultiMapValuesCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MultiMapValuesCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
