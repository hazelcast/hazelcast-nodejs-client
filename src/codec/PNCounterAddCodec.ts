...
export class PNCounterAddCodec {
    ...
    static encodeRequest(name: string, delta: Long, getBeforeUpdate: boolean, replicaTimestamps: Array<[string, Long]>, targetReplica: Address): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName('PNCounter.Add');
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(PNCounterAddCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(value: Long , replicaTimestamps: Array<[string, Long]> , replicaCount: number ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(PNCounterAddCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
