...
export class CacheAddInvalidationListenerCodec {
    ...
    private static EVENT_CACHE_INVALIDATION_SEQUENCE_FIELD_OFFSET = CacheAddInvalidationListenerCodec.EVENT_CACHE_INVALIDATION_PARTITION_UUID_FIELD_OFFSET + FixedSizeTypes.UUID_SIZE_IN_BYTES;
    private static EVENT_CACHE_INVALIDATION_INITIAL_FRAME_SIZE = CacheAddInvalidationListenerCodec.EVENT_CACHE_INVALIDATION_SEQUENCE_FIELD_OFFSET + FixedSizeTypes.LONG_SIZE_IN_BYTES;
    // hex: 0x150202
    private static EVENT_CACHE_INVALIDATION_MESSAGE_TYPE = 1376770;
    ...
    // hex: 0x150203
    private static EVENT_CACHE_BATCH_INVALIDATION_MESSAGE_TYPE = 1376771;
    ...
    static encodeRequest(name: string, localOnly: boolean): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(CacheAddInvalidationListenerCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: string ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(CacheAddInvalidationListenerCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeCacheInvalidationEvent( name: string,  key: Data,  sourceUuid: string,  partitionUuid: UUID,  sequence: Long): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(CacheAddInvalidationListenerCodec.EVENT_CACHE_INVALIDATION_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeCacheBatchInvalidationEvent( name: string,  keys: Array<Data>,  sourceUuids: Array<string>,  partitionUuids: Array<UUID>,  sequences: Array<Long>): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(CacheAddInvalidationListenerCodec.EVENT_CACHE_BATCH_INVALIDATION_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        initialFrame.flags |= ClientMessage.IS_EVENT_FLAG;
    ...
    static handle(clientMessage: ClientMessage,  handleCacheInvalidation: any, handleCacheBatchInvalidation: any, toObjectFunction: (data: Data) => any = null): void {
        ...
        const frame: Frame = clientMessage.get();
        const initialFrame: Frame = frame.next;
        const partitionUuid: UUID  = FixedSizeTypes.decodeUUID(initialFrame.content, CacheAddInvalidationListenerCodec.EVENT_CACHE_INVALIDATION_PARTITION_UUID_FIELD_OFFSET);
        const sequence: Long  = FixedSizeTypes.decodeLong(initialFrame.content, CacheAddInvalidationListenerCodec.EVENT_CACHE_INVALIDATION_SEQUENCE_FIELD_OFFSET);
