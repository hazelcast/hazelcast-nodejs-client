...
export class ScheduledExecutorGetResultFromAddressCodec {
    // hex: 0x1D1000
    public static REQUEST_MESSAGE_TYPE = 1904640;
    // hex: 0x1D1001
    public static RESPONSE_MESSAGE_TYPE = 1904641;
    ...
    static encodeRequest(schedulerName: string, taskName: string, address: Address): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ScheduledExecutorGetResultFromAddressCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ScheduledExecutorGetResultFromAddressCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: Data ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ScheduledExecutorGetResultFromAddressCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ScheduledExecutorGetResultFromAddressCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
