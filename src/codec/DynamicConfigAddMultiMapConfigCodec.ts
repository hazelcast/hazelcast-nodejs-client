...
export class DynamicConfigAddMultiMapConfigCodec {
    ...
    static encodeRequest(name: string, collectionType: string, listenerConfigs: Array<ListenerConfigHolder>, binary: boolean, backupCount: number, asyncBackupCount: number, statisticsEnabled: boolean, splitBrainProtectionName: string, mergePolicy: string, mergeBatchSize: number): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName('DynamicConfig.AddMultiMapConfig');
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(DynamicConfigAddMultiMapConfigCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(DynamicConfigAddMultiMapConfigCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
