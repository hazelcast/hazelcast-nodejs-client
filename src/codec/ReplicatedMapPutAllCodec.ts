...
export class ReplicatedMapPutAllCodec {
    //hex: 0x0E0800
    public static REQUEST_MESSAGE_TYPE = 919552;
    //hex: 0x0E0801
    public static RESPONSE_MESSAGE_TYPE = 919553;
    ...
    static encodeRequest(name: string, entries: Array<[Data,Data]>) {
        var clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName("ReplicatedMap.PutAll");
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(ReplicatedMapPutAllCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ReplicatedMapPutAllCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse() {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(ReplicatedMapPutAllCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ReplicatedMapPutAllCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
