...
export class ClientStatisticsCodec {
    // hex: 0x001000
    public static REQUEST_MESSAGE_TYPE = 4096;
    // hex: 0x001001
    public static RESPONSE_MESSAGE_TYPE = 4097;
    ...
    static encodeRequest(stats: string): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientStatisticsCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ClientStatisticsCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
        ...
        frame = frame.next;
        request.stats = StringCodec.decode(frame);
        return request;
    }

     static encodeResponse(): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ClientStatisticsCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ClientStatisticsCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
