...
export class MapEntriesWithPredicateCodec {
    // hex: 0x012C00
    public static REQUEST_MESSAGE_TYPE = 76800;
    // hex: 0x012C01
    public static RESPONSE_MESSAGE_TYPE = 76801;
    ...
    static encodeRequest(name: string, predicate: Data): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapEntriesWithPredicateCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapEntriesWithPredicateCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: Array<[Data, Data]> ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapEntriesWithPredicateCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapEntriesWithPredicateCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
