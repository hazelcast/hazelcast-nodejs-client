...
export class ContinuousQueryPublisherCreateWithValueCodec {
    ...
    static encodeRequest(mapName: string, cacheName: string, predicate: Data, batchSize: number, bufferSize: number, delaySeconds: Long, populate: boolean, coalesce: boolean) {
        var clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(true);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName("ContinuousQuery.PublisherCreateWithValue");
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(ContinuousQueryPublisherCreateWithValueCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse(response: Array<[Data,Data]> ) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(ContinuousQueryPublisherCreateWithValueCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
