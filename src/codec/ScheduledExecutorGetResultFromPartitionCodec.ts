...
export class ScheduledExecutorGetResultFromPartitionCodec {
    // hex: 0x1D0F00
    public static REQUEST_MESSAGE_TYPE = 1904384;
    // hex: 0x1D0F01
    public static RESPONSE_MESSAGE_TYPE = 1904385;
    ...
    static encodeRequest(schedulerName: string, taskName: string): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ScheduledExecutorGetResultFromPartitionCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ScheduledExecutorGetResultFromPartitionCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: Data ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(ScheduledExecutorGetResultFromPartitionCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ScheduledExecutorGetResultFromPartitionCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
