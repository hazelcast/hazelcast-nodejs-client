...
export class SetAddListenerCodec {
    ...
    private static EVENT_ITEM_INITIAL_FRAME_SIZE = SetAddListenerCodec.EVENT_ITEM_EVENT_TYPE_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    // hex: 0x060B02
    private static EVENT_ITEM_MESSAGE_TYPE = 396034;
    ...
    static encodeRequest(name: string, includeValue: boolean, localOnly: boolean): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(SetAddListenerCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: string ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(SetAddListenerCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeItemEvent( item: Data,  uuid: string,  eventType: number): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(SetAddListenerCodec.EVENT_ITEM_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static handle(clientMessage: ClientMessage,  handleItem: any, toObjectFunction: (data: Data) => any = null): void {
        ...
        const frame: Frame = clientMessage.get();
        const initialFrame: Frame = frame.next;
        const eventType: number  = FixedSizeTypes.decodeInt(initialFrame.content, SetAddListenerCodec.EVENT_ITEM_EVENT_TYPE_FIELD_OFFSET);
