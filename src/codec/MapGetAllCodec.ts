...
export class MapGetAllCodec {
    // hex: 0x012700
    public static REQUEST_MESSAGE_TYPE = 75520;
    // hex: 0x012701
    public static RESPONSE_MESSAGE_TYPE = 75521;
    ...
    static encodeRequest(name: string, keys: Array<Data>): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName('Map.GetAll');
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapGetAllCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapGetAllCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: Array<[Data,Data]> ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapGetAllCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapGetAllCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
