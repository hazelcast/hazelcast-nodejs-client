...
export class MapValuesWithPagingPredicateCodec {
    // hex: 0x013900
    public static REQUEST_MESSAGE_TYPE = 80128;
    // hex: 0x013901
    public static RESPONSE_MESSAGE_TYPE = 80129;
    ...
    static encodeRequest(name: string, predicate: Data): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapValuesWithPagingPredicateCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapValuesWithPagingPredicateCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: Array<[Data,Data]> ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapValuesWithPagingPredicateCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapValuesWithPagingPredicateCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
