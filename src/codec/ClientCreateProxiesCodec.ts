...
import {StringCodec} from '../builtin/StringCodec'
   ...
   export class RequestParameters {
         ...
         */
        public proxies: Array<[string,string]>;
    };
...
export class ClientCreateProxiesCodec {
    //hex: 0x001300
    public static REQUEST_MESSAGE_TYPE = 4864;
    //hex: 0x001301
    public static RESPONSE_MESSAGE_TYPE = 4865;
    ...
    static encodeRequest(proxies: Array<[string, string]>) {
        var clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName("Client.CreateProxies");
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(ClientCreateProxiesCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ClientCreateProxiesCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
        ...
        //empty initial frame
        frame = frame.next;
        request.proxies = MapCodec.decode(frame, StringCodec.decode, StringCodec.decode);
        return request;
    }
     ...
     static encodeResponse() {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(ClientCreateProxiesCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, ClientCreateProxiesCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
