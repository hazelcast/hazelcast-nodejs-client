...
export class QueueAddListenerCodec {
    ...
    static EVENT_ITEM_INITIAL_FRAME_SIZE = QueueAddListenerCodec.EVENT_ITEM_EVENT_TYPE_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    //hex: 0x031102
    static EVENT_ITEM_MESSAGE_TYPE = 200962;
    ...
    static encodeRequest(name: string, includeValue: boolean, localOnly: boolean) {
        ...
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(QueueAddListenerCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse(response: string ) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(QueueAddListenerCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodeItemEvent( item : Data,  uuid : string,  eventType : number) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(QueueAddListenerCodec.EVENT_ITEM_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
...
static handle(clientMessage : ClientMessage,  handleItem: any, toObjectFunction: (data: Data) => any = null) {
            ...
            if (messageType == QueueAddListenerCodec.EVENT_ITEM_MESSAGE_TYPE) {
                var initialFrame : Frame = frame.next;
                var eventType : number  = FixedSizeTypes.decodeInt(initialFrame.content, QueueAddListenerCodec.EVENT_ITEM_EVENT_TYPE_FIELD_OFFSET);
