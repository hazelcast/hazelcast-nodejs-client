...
export class DynamicConfigAddFlakeIdGeneratorConfigCodec {
    ...
    static encodeRequest(name: string, prefetchCount: number, prefetchValidity: Long, idOffset: Long, statisticsEnabled: boolean, nodeIdOffset: Long): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName('DynamicConfig.AddFlakeIdGeneratorConfig');
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(DynamicConfigAddFlakeIdGeneratorConfigCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(DynamicConfigAddFlakeIdGeneratorConfigCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
