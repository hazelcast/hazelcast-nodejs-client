...
export class CacheSetExpiryPolicyCodec {
    // hex: 0x152300
    public static REQUEST_MESSAGE_TYPE = 1385216;
    // hex: 0x152301
    public static RESPONSE_MESSAGE_TYPE = 1385217;
    ...
    private static RESPONSE_INITIAL_FRAME_SIZE = CacheSetExpiryPolicyCodec.RESPONSE_RESPONSE_FIELD_OFFSET + FixedSizeTypes.BOOLEAN_SIZE_IN_BYTES;
    ...
    static encodeRequest(name: string, keys: Array<Data>, expiryPolicy: Data): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(CacheSetExpiryPolicyCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, CacheSetExpiryPolicyCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
        ...
        request.expiryPolicy = DataCodec.decode(frame);
        return request;
    }

     static encodeResponse(response: boolean ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(CacheSetExpiryPolicyCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, CacheSetExpiryPolicyCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);

        FixedSizeTypes.encodeBoolean(initialFrame.content, CacheSetExpiryPolicyCodec.RESPONSE_RESPONSE_FIELD_OFFSET, response);
        return clientMessage;
    }
