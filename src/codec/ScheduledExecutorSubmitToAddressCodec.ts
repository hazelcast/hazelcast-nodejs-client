...
export class ScheduledExecutorSubmitToAddressCodec {
    ...
    static encodeRequest(schedulerName: string, address: Address, type: number, taskName: string, task: Data, initialDelayInMillis: Long, periodInMillis: Long) {
        var clientMessage = ClientMessage.createForEncode();
        clientMessage.setRetryable(true);
        clientMessage.setAcquiresResource(false);
        clientMessage.setOperationName("ScheduledExecutor.SubmitToAddress");
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(ScheduledExecutorSubmitToAddressCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse() {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(ScheduledExecutorSubmitToAddressCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
