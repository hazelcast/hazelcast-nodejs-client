...
export class ClientAddPartitionListenerCodec {
    ...
    static EVENT_PARTITIONS_PARTITION_STATE_VERSION_FIELD_OFFSET = ClientMessage.PARTITION_ID_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    static EVENT_PARTITIONS_INITIAL_FRAME_SIZE = ClientAddPartitionListenerCodec.EVENT_PARTITIONS_PARTITION_STATE_VERSION_FIELD_OFFSET + FixedSizeTypes.INT_SIZE_IN_BYTES;
    //hex: 0x001202
    static EVENT_PARTITIONS_MESSAGE_TYPE = 4610;
    ...
    static encodeRequest() {
        ...
        var initialFrame : Frame= new Frame(Buffer.allocUnsafe(ClientAddPartitionListenerCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
    ...
    static decodeRequest(clientMessage : ClientMessage) {
     ...
     static encodeResponse() {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(ClientAddPartitionListenerCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
     ...
     static encodePartitionsEvent( partitions : Array<[Address, Array<number>]>,  partitionStateVersion : number) {
        var clientMessage = ClientMessage.createForEncode();
        var initialFrame : Frame = new Frame(Buffer.allocUnsafe(ClientAddPartitionListenerCodec.EVENT_PARTITIONS_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
