...
export class MapProjectWithPredicateCodec {
    // hex: 0x014100
    public static REQUEST_MESSAGE_TYPE = 82176;
    // hex: 0x014101
    public static RESPONSE_MESSAGE_TYPE = 82177;
    ...
    static encodeRequest(name: string, projection: Data, predicate: Data): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapProjectWithPredicateCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapProjectWithPredicateCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: Array<Data> ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapProjectWithPredicateCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapProjectWithPredicateCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
     ...
     static decodeResponse(clientMessage: ClientMessage): ResponseParameters {
        ...
        let frame: Frame = clientMessage.get();
        frame = frame.next;
        response.response = ListMultiFrameCodec.decodeContainsNullable(frame, DataCodec.decode);
        return response;
    }
}
