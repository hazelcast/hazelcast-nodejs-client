...
export class MapEntrySetCodec {
    // hex: 0x012900
    public static REQUEST_MESSAGE_TYPE = 76032;
    // hex: 0x012901
    public static RESPONSE_MESSAGE_TYPE = 76033;
    ...
    static encodeRequest(name: string): ClientMessage {
        ...
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapEntrySetCodec.REQUEST_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapEntrySetCodec.REQUEST_MESSAGE_TYPE);
    ...
    static decodeRequest(clientMessage: ClientMessage): RequestParameters {
     ...
     static encodeResponse(response: Array<[Data,Data]> ): ClientMessage {
        const clientMessage = ClientMessage.createForEncode();
        const initialFrame: Frame = new Frame(Buffer.allocUnsafe(MapEntrySetCodec.RESPONSE_INITIAL_FRAME_SIZE), ClientMessage.UNFRAGMENTED_MESSAGE);
        FixedSizeTypes.encodeInt(initialFrame.content, ClientMessage.TYPE_FIELD_OFFSET, MapEntrySetCodec.RESPONSE_MESSAGE_TYPE);
        clientMessage.add(initialFrame);
