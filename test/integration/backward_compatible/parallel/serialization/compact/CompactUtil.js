...
let mimicSchemaReplication;
let createSerializationService;
let createMainDTO;
let validationTestParams;
let referenceObjects;
let varSizeFields;
let nullableFixedSizeFields;
let fixedSizeFields;
let nullableFixedSizeArrayToFixedSizeArray;
let fixedNullableFieldToFixedField;
let fixedFieldToNullableFixedField;
let fixedSizedArrayToNullableFixedSizeArray;
let NodeDTOSerializer;
let NamedDTOSerializer;
let InnerDTOSerializer;
let MainDTOSerializer;
let MainDTOSerializerWithDefaults;
let Flexible;
let FlexibleSerializer;
let supportedFields;
let NodeDTO;
let Bits;
let BitsSerializer;
...
// The imports below will not work for the clients that are older than 5.1.0
if (TestUtil.isClientVersionAtLeast('5.1.0')) {
    ...
    const arrayOfLocalDates = [
        [], [new LocalDate(2022, 12, 12)], [new LocalDate(2022, 12, 12), new LocalDate(2021, 12, 12)], null
    ];
    ...
    const genericRecords = [
        GenericRecords.compact('a',
            {foo: Fields.INT8}, {foo: 1}), null, GenericRecords.compact('b', {bar: Fields.INT16},
            {bar: 2}
        )
    ];
    ...
    const all = [...objects, ...numbers, ...strings, ...arrayOfStrings, ...longs, ...nullableLongs, ...arrayOfLongs,
        ...
        ...arrayOfNullableNumbers, ...arrayOfNullableBooleans, ...arrayOfLocalDates, ...arrayOfLocalTimes,
         ...arrayOfLocalDateTimes, ...arrayOfOffsetDateTimes, ...genericRecords, ...arrayOfGenericRecords
    ];
    ...
    createSerializationService = (compactSerializers = []) => {
    ...
    FlexibleSerializer = class FlexibleSerializer {
    ...
    createMainDTO = () => {
        const now = new Date();
        const getNowLocalTime = () => new LocalTime(
            now.getHours(), now.getMinutes(), now.getSeconds(), now.getMilliseconds() * 1000
        );
        const getNowLocalDate = () => new LocalDate(
        ...
        const inner = new InnerDTO(
            ...
            [Long.fromNumber(0), Long.fromNumber(1), Long.fromNumber(5),
                Long.fromNumber(7), Long.fromNumber(9), Long.fromNumber(11)],
    ...
    createCompactGenericRecord = (mainDTO) => {
    ...
    serialize = async (serializationService, schemaService, obj) => {
        try {
