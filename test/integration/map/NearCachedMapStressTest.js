...
const { getRandomInt } = require('../../TestUtil');
        ...
        client1.getMap(MAP_NAME).then(function (mp) {
            ...
            (function innerOperation() {
                ...
                if (runningOperations >= CONCURRENCY_LEVEL) {
                    ...
                    if (op < PUT_PERCENT) {
                        map.put(getRandomInt(0, NUMBER_OF_ENTRIES), getRandomInt(0, 10000)).then(completeOperation);
                    } else if (op < PUT_PERCENT + REMOVE_PERCENT) {
                        map.remove(getRandomInt(0, NUMBER_OF_ENTRIES)).then(completeOperation);
                    } else {
                        totalGetOperations++;
                        map.get(getRandomInt(0, NUMBER_OF_ENTRIES)).then(completeOperation);
                    }
